...
<div class="modebar">  
  <button onclick="setMode('view')">📖 浏览模式</button>  
  <button onclick="setMode('spell')">✍ 拼写模式</button>  
</div>  
  
<button onclick="start()">开始今日打卡</button>  
...
async function showWord(){  
  ...
  if(mode==="view"){  
    ...
    box.innerHTML=`  
      ...
      <div>中文：${detail.cn}</div>  
      <div class="example" onclick="speak('${detail.en}')">${detail.en}<br>${detail.cnEx}</div>  
      <input id="spellInput" placeholder="输入完整单词">  
      <button onclick="checkSpell()">提交</button>  
    `;  
  }  
  
  document.getElementById("progress").innerText=`进度：${index+1} / ${todayList.length}`;  
}  
...
function completePunch(){  
  ...
  document.getElementById("card").innerHTML="🎉 今日打卡完成！";  
  document.getElementById("progress").innerText=`累计打卡：${rec.length} 天`;  
  showRecord();  
  firework();  
}  
